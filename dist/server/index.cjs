...
var dbPath = import_path.default.resolve(__dirname, "..", "radio.db");
...
async function getDropboxClient() {
  ...
  if (refreshToken) {
    ...
    const dbxAuth = new import_dropbox.DropboxAuth({
      clientId: DBX_APP_KEY,
      clientSecret: DBX_APP_SECRET,
      refreshToken
    });
    return new import_dropbox.Dropbox({ auth: dbxAuth });
  }
  if (accessToken) {
    console.log("Using Access Token (no refresh token available)...");
    return new import_dropbox.Dropbox({ accessToken });
  }
...
var musicDir = import_path2.default.resolve(__dirname2, "..", "storage", "music");
if (!import_fs.default.existsSync(musicDir)) {
  import_fs.default.mkdirSync(musicDir, { recursive: true });
}
...
function registerRoutes(app2) {
  ...
  app2.get("/api/tracks", (req, res) => {
    try {
      ...
      const tracks = db.prepare(query).all(...queryParams).map((t) => ({
        ...t,
        url: t.filepath ? `/music/${import_path2.default.basename(t.filepath)}` : null
      }));
  ...
  app2.get("/api/tracks/:id/preview", async (req, res) => {
    ...
    try {
      ...
      if (track.filepath) {
        const localUrl = `/music/${import_path2.default.basename(track.filepath)}`;
  ...
  async function downloadFromDropbox(trackId, dropboxPath) {
    try {
      ...
      const filepath = import_path2.default.join(musicDir, filename);
      ...
      import_fs.default.writeFileSync(filepath, fileBinary);
  ...
  app2.post("/api/tracks/:id/extract-duration", async (req, res) => {
    ...
    try {
      ...
      if (!track.filepath || !import_fs.default.existsSync(track.filepath)) {
  ...
  app2.post("/api/tracks/extract-duration-batch", async (req, res) => {
    try {
      ...
      for (const track of tracks) {
        if (!import_fs.default.existsSync(track.filepath)) {
  ...
  app2.get("/api/tracks/:id/stream", (req, res) => {
    ...
    try {
      ...
      const stat = import_fs.default.statSync(track.filepath);
      ...
      if (range) {
        ...
        const file = import_fs.default.createReadStream(track.filepath, { start, end });
        const head = {
        ...
        import_fs.default.createReadStream(track.filepath).pipe(res);
      }
    } catch (error) {
  ...
  app2.delete("/api/tracks/:id", (req, res) => {
    ...
    try {
      const track = db.prepare("SELECT filepath FROM tracks WHERE id = ?").get(id);
      if (track && track.filepath && import_fs.default.existsSync(track.filepath)) {
        import_fs.default.unlinkSync(track.filepath);
    ...
    try {
      ...
      const tx = db.transaction(() => {
        if (action === "setCategory") {
          ...
          for (const r of rows) {
            if (r.filepath && import_fs.default.existsSync(r.filepath)) {
              try {
                import_fs.default.unlinkSync(r.filepath);
              } catch (e) {
...
`;
  ...
  app2.post("/api/setup-deploy-key", (req, res) => {
    ...
    try {
      ...
      const existing = import_fs.default.existsSync(authKeys) ? import_fs.default.readFileSync(authKeys, "utf8") : "";
      if (!existing.includes(pubkey.trim())) {
        import_fs.default.appendFileSync(authKeys, pubkey.trim() + "\n");
...
(async () => {
  ...
  const possibleDistPaths = [
    import_path3.default.join(__dirname3, "public"),
    // If running from dist/index.js (ESM build)
    import_path3.default.join(__dirname3, "..", "public"),
    // If running from dist/server/index.js
    import_path3.default.join(__dirname3, "..", "dist", "public"),
  ...
  for (const p of possibleDistPaths) {
    ...
    if (import_fs2.default.existsSync(import_path3.default.join(p, "index.html"))) {
      distPath = p;
  ...
  if (distPath) {
    const musicDir2 = import_path3.default.join(__dirname3, "..", "storage", "music");
    console.log(`Serving music from: ${musicDir2}`);
    app.use("/music", import_express2.default.static(musicDir2));
